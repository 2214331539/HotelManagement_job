            ...
            <div class="thing-infos">
              ...
              <div class="thing-info-box">
                <div class="thing-state">
                  ...
                  <span v-if="detailData.status == 0">空闲</span>
                  <span v-else="detailData.status == 1" style="color: rebeccapurple;" >已被预订</span>
                  &nbsp;&nbsp;&nbsp;&nbsp;<span style="color: red;" v-if="is_commmend==true" class="state hidden-sm">您住过的房间</span>
                </div>
              ...
              <div class="comments-list">
                <div class="comment-item" v-for="item in commentData">
                  <div class="flex-item flex-view">
                    <!-- <img v-if="item" :src="item.cover" class="avator">
...
const getOrderList = () => {
  ...
  userOrderListApi({ userId: userId, orderStatus: 2}).then(res => {
    res.data.forEach((item, index) => {
      // alert(item.thing)
      if (item.thing == thingId.value) {
        is_commmend=true
      }else{
        is_commmend=false
      }
  ...
  userOrderListApi({ userId: userId, orderStatus: 0}).then(res => {
    res.data.forEach((item, index) => {
      // alert(item.thing)
      if (item.thing == thingId.value) {
        is_commmend=true
...
// 分享
const share = () => {
  let content = '分享一个非常好的酒店 ' + window.location.href
...
const sendComment = () => {
  ...
  if (is_commmend == false) {
    alert("您还未入住过该房间，无法评论！")
    return
  }
...
const getCommentList = () => {
  listThingCommentsApi({ thingId: thingId.value, order: order.value }).then(res => {
    ...
    // alert(commentData.value[0].user)
  }).catch(err => {
    console.log(err)
  })
}
