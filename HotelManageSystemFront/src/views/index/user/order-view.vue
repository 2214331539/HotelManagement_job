      ...
      <div class="list-content">
        <div class="order-item-view" v-for="(item, index) in orderData" :key="index">
          <div class="header flex-view">
            ...
            <div class="right">
              <a-popconfirm v-if="item.status === '1'" title="确定取消订单？" ok-text="是" cancel-text="否"
                ...
                <a-button type="primary" size="small" style="margin-right: 24px;">取消订单</a-button>
              </a-popconfirm>

              <a-popconfirm v-if="item.status === '1'" title="确定模拟支付订单？" ok-text="是" cancel-text="否"
                @confirm="handlePay(item)">
                <a-button type="primary" size="small" style="margin-right: 24px;">模拟支付</a-button>
              </a-popconfirm>

              <span class="text">订单状态</span>
              <span class="state">{{ item.status === '0' ? '已完成' : item.status === '1' ? '待支付' : item.status === '2' ? '已支付' : '已取消' }}</span>
            </div>
          </div>
          ...
          <div class="bottom flex-view">
            <div class="left">
              <span class="text">共{{ item.count }}间</span>
              <span class="open" @click="handleDetail(item.thing)">房间详情</span>
            </div>
